  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Meus Alunos</h1>
          <p className="text-gray-500 mt-1">Gerencie todos os seus alunos</p>
        </div>
                ...
                {alunosComStats.map((aluno) => (
                  <tr key={aluno.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-2 text-gray-500">
                      {aluno.ultimaMetrica ? `${aluno.ultimaMetrica.peso} kg` : "—"}
                    </td>
                    <td className="py-3 px-2">
                      {aluno.pagamentosPendentes > 0 ? (
                        <Badge variant="warning">{aluno.pagamentosPendentes} pendente(s)</Badge>
                      ) : (
