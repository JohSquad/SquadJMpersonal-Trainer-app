...
import { registerAluno } from "@/lib/actions/auth";

export default function RegisterAlunoPage() {
  const [state, formAction, pending] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      return (await registerAluno(formData)) ?? null;
    },
    null
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-900 to-blue-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Cadastro Aluno</h1>
          <p className="text-gray-500 mt-1">
            Use o código de convite do seu personal trainer
          </p>
        </div>
